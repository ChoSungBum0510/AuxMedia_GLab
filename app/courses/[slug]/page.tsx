...
export default async function CourseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <><section className="course-detail-hero" style={{ "--region-color": region.color, "--region-pale": region.paleColor } as React.CSSProperties}><div className="shell course-detail-hero__inner"><div><RegionBadge region={region} /><div className="course-detail-tags"><span className={`status status--${course.status}`}>{courseStatusLabel(course.status)}</span><span>{course.category}</span><span>{course.format}</span></div><h1>{course.title}</h1><p>{course.summary}</p></div><aside><span>모집 안내</span><strong>{display.application}</strong><small>{display.participation}</small>{course.status === "open" ? <Link className="button button--wide" href={`/apply/${course.slug}`}>이 교육 신청하기</Link> : <Link className="button button--wide" href="/notices">알림마당 확인하기</Link>}</aside></div></section>
      ...
      <section className="course-final-cta" style={{ "--region-color": region.color } as React.CSSProperties}><div className="shell"><div><span>{region.koreanName} GLab</span><h2>단회성 체험에서<br />지속형 배움으로.</h2></div><div><p>운영 안내: {region.contact}</p>{course.status === "open" ? <Link className="button button--white" href={`/apply/${course.slug}`}>지금 신청하기 →</Link> : <Link className="button button--white" href="/notices">새 일정 확인하기 →</Link>}</div></div></section></>
  );
}
