...
export default async function AdminPage() {
  ...
  if (!user) return <section className="auth-gate auth-gate--admin"><div><BrandLogo className="official-brand--auth" /><h1>관리자 로그인</h1><p>교육과정, 신청자와 수강후기를 통합 관리합니다.</p><Link className="button button--wide" href={signInPath("/admin", "admin")}>관리자 계정으로 로그인</Link></div></section>;
