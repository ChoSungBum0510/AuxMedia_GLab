...
export async function generateMetadata({ params }: { params: Promise<{ region: string }> }): Promise<Metadata> {
  ...
  return region ? { title: region.displayName, description: region.description } : { title: "지역 교육 거점" };
}
  ...
  return (
    <>
      ...
      <section className="region-overview"><div className="shell region-overview__inner">
        <div><span className="eyebrow">UNIVERSITY × COMMUNITY</span><h2>지역 현안을 발견하고,<br />교육과 협업으로 연결합니다.</h2><p>한림대학교의 지역 교육 거점은 지역의 수요와 대학의 역량을 연결해 실제 프로젝트와 지속형 배움으로 이어지게 합니다.</p></div>
      ...
      <section className="section" id="programs"><div className="shell"><div className="section-title-row"><SectionHeading eyebrow={`${region.koreanName.toUpperCase()} PROGRAMS`} title={`${region.displayName} 교육·활동`} description="운영을 마친 과정과 예정 프로그램을 안내합니다." /><Link href="/courses" className="text-link text-link--large">전체 지역 과정 →</Link></div>{courses.length ? <div className="course-grid">{courses.map((course) => <CourseCard key={course.slug} course={course} />)}</div> : <div className="empty-state"><strong>공개된 교육 일정이 아직 없습니다.</strong><p>{region.displayName}의 세부 사업과 모집 일정은 확정 후 알림마당에서 안내합니다.</p><Link className="button" href="/notices">알림마당 확인하기 →</Link></div>}</div></section>
      <section className="section section--pale"><div className="shell region-focus-layout">
        ...
        <div className="region-photo-card" style={{ backgroundImage: `url(${region.image})`, backgroundPosition: region.imagePosition }}><div><span>{region.displayName}</span><strong>지역의 필요를<br />지속형 변화로</strong></div></div>
      </div></section>
      <section className="section"><div className="shell next-regions"><div><span className="eyebrow">EXPLORE MORE</span><h2>다른 지역 교육 거점도 살펴보세요.</h2></div>{regions.filter((item) => item.slug !== region.slug).map((item) => <Link key={item.slug} href={`/regions/${item.slug}`} style={{ "--region-color": item.color } as React.CSSProperties}><RegionBadge region={item} compact /><span>{item.headline}</span><b>↗</b></Link>)}</div></section>
    </>
  );
}
