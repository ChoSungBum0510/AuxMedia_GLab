  ...
  return (
    <>
      ...
      <section className="region-overview"><div className="shell region-overview__inner">
        <div><span className="eyebrow">UNIVERSITY × COMMUNITY</span><h2>지역 현안을 발견하고,<br />교육과 협업으로 연결합니다.</h2><p>G-Lab은 지자체 정책연구, 지역 기업 협업, 교과·비교과 교육을 연결해 지역의 필요가 실제 프로젝트와 지속형 배움으로 이어지게 합니다.</p></div>
      ...
      <section className="section" id="programs"><div className="shell"><div className="section-title-row"><SectionHeading eyebrow={`${region.koreanName.toUpperCase()} PROGRAMS`} title={`${region.koreanName} GLab 교육·활동`} description="공식 운영자료로 확인된 과정과 예정 프로그램을 안내합니다." /><Link href="/courses" className="text-link text-link--large">전체 지역 과정 →</Link></div>{courses.length ? <div className="course-grid">{courses.map((course) => <CourseCard key={course.slug} course={course} />)}</div> : <div className="empty-state"><strong>공개된 교육 일정이 아직 없습니다.</strong><p>{region.koreanName} GLab의 세부 사업과 모집 일정은 확정 후 알림마당에서 안내합니다.</p><Link className="button" href="/notices">알림마당 확인하기 →</Link></div>}</div></section>
