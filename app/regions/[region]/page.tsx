  ...
  return (
    <>
      <section className="region-hero" style={{ "--region-color": region.color } as React.CSSProperties}>
        ...
        <div className="shell region-hero__content"><RegionBadge region={region} /><span className="hero-kicker">{region.eyebrow}</span><h1>{region.headline}</h1><p>{region.description}</p><div className="hero-actions"><Link href="#programs" className="button button--white">교육과정 보기 ↓</Link><Link href={`/courses?region=${region.slug}`} className="button button--glass">전체 과정 →</Link></div></div>
      </section>
      <section className="region-overview"><div className="shell region-overview__inner">
        <div><span className="eyebrow">LOCAL LEARNING HUB</span><h2>{region.koreanName}에서 배우고,<br />{region.koreanName}을 위해 만듭니다.</h2><p>대학의 전문성과 지역의 경험을 결합해 학습이 실제 프로젝트와 새로운 기회로 이어지도록 설계합니다.</p></div>
      ...
      <section className="section section--pale"><div className="shell region-focus-layout">
        <div><SectionHeading eyebrow="WHAT WE FOCUS ON" title="지역을 닮은 배움" description={`${region.koreanName}의 자원과 과제를 교육 프로젝트로 연결합니다.`} /><div className="focus-tags">{region.focus.map((focus, index) => <span key={focus}><b>0{index + 1}</b>{focus}</span>)}</div></div>
        <div className="region-photo-card" style={{ backgroundImage: `url(${region.image})`, backgroundPosition: region.imagePosition }}><div><span>{region.koreanName} GLab</span><strong>현장에서 시작해<br />지역 안에 남는 교육</strong></div></div>
      </div></section>
